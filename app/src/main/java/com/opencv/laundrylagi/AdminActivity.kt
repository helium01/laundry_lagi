...
import com.opencv.laundrylagi.fragment.admin.*
import com.opencv.laundrylagi.helper.SharePref

class AdminActivity : AppCompatActivity() {
    private val FragmentHome : Fragment = HomeFragment()
    private val FragmentAkun : Fragment = ProfilFragment()
    private val FragmentPaketLaundry : Fragment = LaundryFragment()
    private val FragmentPemasukan : Fragment = OrderFragment()
    private val FragmentKupon : Fragment = KuponFragment()
    ...
    fun setButtonNav(){
        ...
        fm.beginTransaction().add(R.id.container1,FragmentKupon).hide(FragmentKupon).commit()
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener { item->
            when(item.itemId){
                ...
                R.id.navigation_progres->{
                    panggilfragment(3,FragmentKupon)
                }
                R.id.navigation_user->{
                    panggilfragment(4,FragmentAkun)
                }
                R.id.navigation_pembayaran->{
                    panggilfragment(2,FragmentPemasukan)
                }
